...
- **보관함**(`pantry` 컬렉션, 별도 탭) — 산 물건과 유통기한을 D-day로 본다. 지출과 별개라 한쪽을 지워도 다른 쪽은 남는다. 유통기한은 `/*EXPIRY_START*/` 구간이 정한다: 텍스트에 적힌 실제 날짜 → 이름 키워드 추정(`guessed:true`, 화면에 "기한 추정" 표시) → 모르면 빈칸. **모르는 이름에 기본값을 채우지 않는다** — 멀쩡한 걸 버리거나 상한 걸 먹게 만든다. 닫힌 페이지는 알림을 못 보내므로 열었을 때 배너·탭 배지로만 알린다

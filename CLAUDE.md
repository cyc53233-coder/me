...
`npm test` 는 **`data-audit.mjs` 를 먼저 돌린다** — 픽스처가 아니라 배포되는 진짜
`data/deals.js` 를 훑어, 등급표 밖의 `grade`·설정에 없는 `category`·중복 링크·미래 날짜·
평소가가 지금가보다 싼 경우를 잡는다. 화면은 멀쩡한데 데이터만 틀어진 상태가 제일 늦게 발견된다.
`check.mjs` 는 브라우저를 띄우기 전에 **설정과 이슈 폼이 어긋났는지** 먼저 대조한다.

**미리보기 사진**(`assets/og.png`)은 `assets/og.html` 을 브라우저로 찍어 만든 생성물이다.
손으로 고치지 말고 `cd site-test && npm run og`. 카톡·인스타는 `og:image` 가 절대주소여야 그림을
그리므로 `app.js` 가 `site.js` 의 `ogImage` 를 절대주소로 바꿔 넣는다 — 상대주소로 두면 조용히 안 뜬다.

**딜 상태는 세 가지** — 파는 중 / `soldOut` 품절 / `ended` 마감. 품절·마감은 `site.js` 의
`endedHours`(48시간) 동안 배지로 남았다가 목록에서 빠지고, 그동안 맨 아래로 내려가며 공유
버튼도 달리지 않는다. 놓친 딜이 보여야 알림방에 들어올 이유가 생긴다. 이슈 폼 「딜 마감 · 품절」
에서 둘 중 하나를 고른다.

**`scripts/deals-file.mjs` 의 `KEY_ORDER` 에 없는 키는 저장할 때 조용히 버려진다.** 실제로
`endedAt` 이 빠져 있어 "마감 48시간 표시" 가 한동안 동작하지 않았고, 픽스처에 값을 손으로 적어
넣는 화면 검사로는 잡히지 않았다. 새 필드를 쓰기 시작하면 목록에 넣고, `site-test/end-state.mjs`
가 파일 왕복을 확인한다.

**카드 안 너비는 178px 뿐이다.** 등급(🔥 5개)과 가격을 한 줄에 두면 가격이 공유 버튼 밑으로
잘린다. 그래서 등급은 시각과 같은 윗줄에 작은 글자로, 가격은 아래 줄에 크게 둔다. 이 줄에 무언가
더 넣으려면 `npm test` 의 "가격이 잘리거나 공유 버튼에 겹치지 않음" 과 "카드 높이가 고르게
유지됨" 을 같이 볼 것 — 한 카드만 줄이 넘으면 그 카드만 키가 커진다.
...
**🔥 개수는 5 / 3 / 1 이다** (무지성급 / 대박 / 추천). `data/site.js` 의 `grades` 가 유일한 기준이고
딜 데이터의 `grade` 값이 곧 그 숫자다. 개수를 또 바꾸면 **이슈 폼(`deal-add.yml`)의 🔥 개수도 같이**
고쳐야 한다 — `add-deal.mjs` 는 드롭다운의 🔥 를 세서 값을 정하므로, 둘이 어긋나면 등급이 조용히 틀어진다.

**카테고리는 `{name, icon}` 객체다** (신선식품 · 간식·음료 · 생활용품 · 기타). 아이콘은 화면에서만 붙고
저장되는 값은 이름뿐이다 (`categoryName()` 이 폼 값에서 아이콘을 뗀다). 폰 폭 390px 에서 칩 5개가
간신히 들어가므로(362px 필요, 362px 있음) **카테고리를 더 늘리면 가로로 넘친다** — 늘릴 거면
아이콘을 이름 위에 얹은 지금 구조를 유지한 채 이름을 줄이거나, 가로 스크롤을 받아들여야 한다.
...
**쇼핑몰이 주는 사진 주소는 임시 보관함이다** (토스는 `/live/temp/…`). 그대로 두면 쇼핑몰이
지울 때 사진이 한꺼번에 깨지므로, `scripts/save-images.mjs` 가 받아서 `assets/deals/` 에 둔다.
파일 이름은 주소의 해시라 같은 사진을 두 번 받지 않고 경로를 벗어나는 이름도 나올 수 없다.
**이 환경에서는 쇼핑몰이 막혀 있어(403) 여기서는 못 받는다** — GitHub Actions 러너에서 돈다.
「딜 등록」이 올리는 김에 같이 받고, 그때 못 받은 것은 「딜 사진 내려받기」(수동 실행 · 매주 월요일)가
다시 받는다. 못 받은 딜은 원래 주소를 그대로 두므로 사진이 사라지지는 않는다.
...
`kakaoText()` 하나가 유일한 출처다 — 사이트의 공유 버튼도, 딜 등록 워크플로가 이슈에 남기는
답글도 같은 함수를 쓴다 (`loadBrowserContext()` 가 `assets/app.js` 를 그대로 읽는다).
순서는 **고지 → 상품 → 안내**이고, 고지가 맨 위인 것은 공정위 표시가 스크롤 없이 보여야 하는데
카톡이 긴 글을 접어 버리기 때문이다. 첫 줄은 `site.js` 의 `shareIntro` 다 — `disclosure` 는
"이 사이트의…" 로 시작해 메시지에는 안 맞아서 따로 둔다. **둘 다 지우지 말 것.**

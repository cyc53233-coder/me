...
- **보관함 (유통기한 D-day)**
  - 영수증·주문 스크린샷에서 뽑은 품목을 지출과 함께 보관함에 담는다 (식품으로 보이는 것만 기본 체크)
  - 유통기한은 텍스트에 적힌 날짜를 우선 쓰고, 없으면 이름으로 추정해 **"기한 추정"이라고 표시**한다. 모르는 이름은 비워 둔다
  - 임박한 순으로 정렬, D-day 배지 · 상단 배너 · 탭 배지. "다 먹었어요"로 치우고 되돌리기 가능
  - 페이지가 닫혀 있을 땐 알림을 보낼 수 없어 **열었을 때** 알린다
